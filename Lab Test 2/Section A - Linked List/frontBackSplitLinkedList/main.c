...
void frontBackSplitLinkedList(LinkedList *ll, LinkedList *resultFrontList, LinkedList *resultBackList)
{
	...
	if((ll == NULL) || (resultBackList == NULL) || (frontBackSplitLinkedList == NULL)){
        return;
	}
	...
	if(length < 2){
        insertNode(resultFrontList,0,curr->item);
        resultBackList = NULL;
	}else{
        int hCount;
        hCount = length/2;
        ...
        for(i = 0; i < hCount;i++){
            d = curr->item;
         curr = curr->next;
            insertNode(resultFrontList,resultFrontList->size,d);

        }

        for(i = hCount; i < length;i++){
            d = curr->item;
            curr = curr->next;
            insertNode(resultBackList,resultBackList->size,d);
        }
	}
}
